...
var StateContext = /** @class */ (function () {
  ...
  StateContext.prototype.includeCurrentData = function (data, keys) {
    ...
    var newData = {};
    for (var i = 0; i < keys.length; i++) newData[keys[i]] = this.data[keys[i]];
...
function createFluentNavigator(states, stateHandler, stateContext, rewrite) {
  ...
  function getCrumbTrail(state, navigationData, crumbs, nextCrumb) {
    if (!state.trackCrumbTrail) return [];
    crumbs = crumbs.slice();
    if (nextCrumb) crumbs.push(nextCrumb);
  ...
  function navigateLink(state, data, hash, crumbs, url) {
    ...
    fluentContext.nextCrumb = new Crumb(
      data,
      state,
      url,
      stateHandler.getLink(state, data, hash),
      false,
      hash
    );
  ...
  return {
    ...
    navigate: function (stateKey, navigationData, hash) {
      ...
      var crumbs = stateContext.crumbs,
        nextCrumb = stateContext.nextCrumb;
      if (!state) throw new Error(stateKey + ' is not a valid State');
      if (typeof navigationData === 'function')
        navigationData = navigationData(stateContext.data);
      var url = stateHandler.getLink(
        state,
        navigationData,
        hash,
        crumbs,
      ...
      if (url == null)
        throw new Error(
          'Invalid route data, a mandatory route parameter has not been supplied a value'
        );
    ...
    navigateBack: function (distance) {
      ...
      if (!(distance <= crumbs.length && distance > 0))
        throw new Error(
          'The distance parameter must be greater than zero and less than or equal to the number of Crumbs (' +
            stateContext.crumbs.length +
            ')'
        );
      var _a = crumbs[crumbs.length - distance],
        state = _a.state,
        data = _a.data,
        url = _a.url;
...
var HTML5HistoryManager = /** @class */ (function () {
  ...
  HTML5HistoryManager.prototype.init = function (navigateHistory, rewriteUrl) {
    ...
    if (!this.disabled && !this.navigateHistory) {
      this.navigateHistory = function (e) {
        return navigateHistory(
          (e.state && e.state.navigationLink) || undefined
        );
      };
  ...
  HTML5HistoryManager.prototype.addHistory = function (url, replace) {
    ...
    if (
      !this.disabled &&
      (((_a = window.history.state) === null || _a === void 0
        ? void 0
        : _a.navigationLink) || this.getUrl(window.location)) !== url
    ) {
      if (!replace)
        ...
        window.history.replaceState(
          __assign(__assign({}, window.history.state), { navigationLink: url }),
          null,
          this.getHref(url)
        );
    }
  ...
  HTML5HistoryManager.prototype.getCurrentUrl = function () {
    ...
    return (
      ((_a = window.history.state) === null || _a === void 0
        ? void 0
        : _a.navigationLink) || this.getUrl(window.location)
    );
  };
  HTML5HistoryManager.prototype.getHref = function (url) {
    var _a;
    if (url == null) throw new Error('The Url is invalid');
    return (
      this.applicationPath +
      HTML5HistoryManager.prependSlash(
        ((_a = this.rewriteUrl) === null || _a === void 0
          ? void 0
          : _a.call(this, url)) || url
      )
    );
  };
  HTML5HistoryManager.prototype.getUrl = function (hrefElement) {
    return (
      hrefElement.pathname.substring(this.applicationPath.length) +
      hrefElement.search +
      hrefElement.hash
    );
  };
...
var HashHistoryManager = /** @class */ (function (_super) {
  ...
  function HashHistoryManager(replaceQueryIdentifier) {
    if (replaceQueryIdentifier === void 0) {
      replaceQueryIdentifier = false;
    }
  ...
  HashHistoryManager.prototype.encode = function (url) {
    if (!this.replaceQueryIdentifier) return url;
    return url.replace('?', '#');
  };
  HashHistoryManager.prototype.decode = function (hash) {
    if (!this.replaceQueryIdentifier) return hash;
    return hash.replace('#', '?');
  };
    ...
    if (typeof val === 'string') {
      if (!separable) {
        ...
        for (var i = 0; i < vals.length; i++) {
          ...
          else throw new Error('Empty string is not a valid array item');
        }
      } else {
        if (val.length !== 0) arr.push(this.converter.convertFrom(val));
        else throw new Error('Empty string is not a valid array item');
      }
    } else {
      for (var i = 0; i < val.length; i++) {
        if (val[i].length !== 0) arr.push(this.converter.convertFrom(val[i]));
        else throw new Error('Empty string is not a valid array item');
      }
    }
    return arr;
  ...
  ArrayConverter.prototype.convertTo = function (val) {
    ...
    for (var i = 0; i < val.length; i++) {
      if (val[i] != null && val[i].toString()) {
        ...
        throw new Error(
          'Invalid navigation data, arrays cannnot contain null, undefined or empty string'
        );
      }
    }
...
var DateConverter = /** @class */ (function (_super) {
  ...
  DateConverter.prototype.convertFrom = function (val) {
    ...
    if (dateParts.length !== 3) throw new Error(val + ' is not a valid date');
    var date = new Date(+dateParts[0], +dateParts[1] - 1, +dateParts[2]);
    if (isNaN(+date)) throw new Error(val + ' is not a valid date');
    return date;
  };
...
var ConverterFactory = /** @class */ (function () {
  function ConverterFactory() {
    ...
    var converterArray = [
      new StringConverter('0'),
      new BooleanConverter('1'),
      new NumberConverter('2'),
      new DateConverter('3'),
    ];
    for (var i = 0; i < converterArray.length; i++) {
      ...
      this.keyToConverterList[converter.key] = this.nameToConverterList[
        converter.name
      ] = converter;
      this.keyToConverterList[arrayConverter.key] = this.nameToConverterList[
        arrayConverter.name
      ] = arrayConverter;
    }
  }
  ...
  ConverterFactory.prototype.getConverterFromName = function (name) {
    ...
    if (!converter) throw new Error('No TypeConverter found for ' + name);
    return converter;
  };
...
var NavigationDataManager = /** @class */ (function () {
  ...
  NavigationDataManager.prototype.formatData = function (
    state,
    navigationData,
    crumbTrail
  ) {
  ...
  NavigationDataManager.prototype.formatDataItem = function (
    state,
    key,
    val,
    data,
  ...
  NavigationDataManager.prototype.formatURLObject = function (
    key,
    urlObject,
    state,
    encode
    ...
    if (encode) {
      ...
      if (formattedArray)
        formattedArray[0] = NavigationDataManager.encodeUrlValue(
          formattedArray[0]
        );
    }
    if (state.trackTypes && converter.name !== defaultType) {
      ...
      if (formattedArray)
        formattedArray[0] =
          formattedArray[0] + NavigationDataManager.SEPARATOR + converter.key;
    }
    return { val: formattedValue, arrayVal: formattedArray };
  };
  NavigationDataManager.prototype.parseData = function (
    data,
    state,
    separableData
  ) {
    ...
    for (var key in data) {
      if (
        !NavigationDataManager.isDefault(key, data, state, !!separableData[key])
      )
        newData[key] = this.parseURLString(
          key,
          data[key],
          state,
          false,
  ...
  NavigationDataManager.isDefault = function (key, data, state, separable) {
    ...
    if (!separable || !arrayDefaultVal) {
      ...
      if (val.length !== arrayDefaultVal.length) return false;
      for (var i = 0; i < val.length; i++) {
        if (val[i] !== arrayDefaultVal[i]) return false;
      }
      return true;
    }
  };
  NavigationDataManager.prototype.parseURLString = function (
    key,
    val,
    state,
    decode,
    ...
    var converterKey =
      this.converterFactory.getConverterFromName(defaultType).key;
    if (
      state.trackTypes &&
      urlValue.indexOf(NavigationDataManager.SEPARATOR) > -1
    ) {
      var arr = urlValue.split(NavigationDataManager.SEPARATOR);
      urlValue = arr[0];
      converterKey = arr[1];
    }
    if (decode) urlValue = NavigationDataManager.decodeUrlValue(urlValue);
    if (typeof val === 'string') val = urlValue;
    else val[0] = urlValue;
    return this.converterFactory
      .getConverterFromKey(converterKey)
  ...
  Segment.prototype.parse = function (defaults) {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      if (subSegment.slice(0, 1) === '{') {
        ...
        if (this.path.length === subSegment.length) optional = this.optional;
    ...
    if (!this.optional) this.pattern = '\/' + this.pattern;
  };
...
var Route = /** @class */ (function () {
  ...
  Route.prototype.build = function (data, urlEncode) {
    ...
    for (var i = this.segments.length - 1; i >= 0; i--) {
      ...
      if (!optional) {
        if (pathInfo.path == null) return null;
...
var Router = /** @class */ (function () {
  ...
  Router.prototype.match = function (path, fromRoute, urlDecode) {
    path = path.slice(-1) === '/' ? path.substring(0, path.length - 1) : path;
    path =
      path.substring(0, 1) === '/' || path.length === 0 ? path : '/' + path;
    var startMatching = !fromRoute;
    for (var i = 0; i < this.routes.length; i++) {
      var route = this.routes[i];
      if (startMatching) {
        var data = route.match(path, urlDecode);
        if (data) return { route: route, data: data };
      } else {
...
var StateRouter = /** @class */ (function () {
  ...
  StateRouter.prototype.getData = function (path, fromRoute) {
    ...
    if (splat) {
      for (var i = 0; i < params.length; i++) {
        var param = params[i];
        if (param.splat) separableData[param.name] = true;
      }
    }
    return {
      state: state,
      data: data,
      separableData: separableData,
      route: route,
    };
  };
  StateRouter.prototype.getRoute = function (state, data, arrayData) {
    ...
    for (var key in routeInfo.params) {
      if (data[key]) paramsKey += routeInfo.params[key] + ',';
    }
    paramsKey = paramsKey.slice(0, -1);
    var routeMatch = routeInfo.matches[paramsKey];
    var routePath = null;
    if (routeMatch) {
      var combinedData = StateRouter.getCombinedData(
        routeMatch.route,
        data,
        arrayData
      );
      ...
      var bestMatch = StateRouter.findBestMatch(
        routeInfo.routes,
        data,
        arrayData
      );
    ...
    for (var i = 0; i < routes.length; i++) {
      ...
      if (routePath) {
        ...
        if (
          ...
          (count === bestMatchCount &&
            route.params.length < bestMatchParamCount)
        ) {
  ...
  StateRouter.getCombinedData = function (route, data, arrayData) {
    if (!route._splat) return data;
    var combinedData = {};
    for (var key in data) combinedData[key] = data[key];
    for (var i = 0; i < route.params.length; i++) {
      var param = route.params[i];
      var arr = arrayData[param.name];
      if (param.splat && arr) combinedData[param.name] = arr;
    }
    return combinedData;
  };
  ...
  StateRouter.prototype.addStateRoutes = function (state) {
    ...
    for (var i = 0; i < routes.length; i++) {
      ...
      route._state = state;
      route._splat = splat;
      route.defaults = StateRouter.getCombinedData(
        route,
        state.formattedDefaults,
        state.formattedArrayDefaults
      );
    }
...
var StateHandler = /** @class */ (function () {
  ...
  StateHandler.prototype.buildStates = function (states) {
    ...
    for (var i = 0; i < states.length; i++) {
      ...
      var state = new State();
      for (var key in stateObject) state[key] = stateObject[key];
      if (!state.key) throw new Error('State key is mandatory');
      if (state.route == null) state.route = state.key;
      ...
      for (var key in state.defaults) {
        if (!state.defaultTypes[key])
          state.defaultTypes[key] = this.navigationDataManager.getConverter(
            state.defaults[key]
          ).name;
        var formattedData = this.navigationDataManager.formatURLObject(
          key,
          state.defaults[key],
          state
        );
  ...
  StateHandler.prototype.getLink = function (
    state,
    navigationData,
    hash,
    crumbs,
    ...
    if (crumbs) {
      crumbs = crumbs.slice();
      if (nextCrumb) crumbs.push(nextCrumb);
      crumbs = state.truncateCrumbTrail(
        state,
        __assign(__assign({}, state.defaults), navigationData),
        crumbs
      );
  ...
  StateHandler.prototype.getNavigationLink = function (
    state,
    navigationData,
    hash,
    crumbTrail
  ) {
    var _a = this.navigationDataManager.formatData(
        state,
        navigationData,
        crumbTrail
      ),
      data = _a.data,
      arrayData = _a.arrayData;
    var routeInfo = this.router.getRoute(state, data, arrayData);
    if (routeInfo.route == null) return null;
    var query = [];
    for (var key in data) {
    ...
    if (query.length > 0) routeInfo.route += '?' + query.join('&');
    if (hash) routeInfo.route += '#' + hash;
  ...
  StateHandler.prototype.parseLink = function (url, fromRoute, err) {
    ...
    var path =
      queryIndex < 0 ? pathAndQuery : pathAndQuery.substring(0, queryIndex);
    var query = queryIndex >= 0 ? pathAndQuery.substring(queryIndex + 1) : null;
    var match = this.router.getData(path, fromRoute);
    if (!match)
      throw new Error(
        'The Url ' + url + ' is invalid' + (err || '\nNo match found')
      );
    var state = match.state,
      data = match.data,
      separableData = match.separableData,
      route = match.route;
    try {
  ...
  StateHandler.prototype.getNavigationData = function (
    query,
    state,
    data,
    separableData
    ...
    if (query) {
      ...
      for (var i = 0; i < params.length; i++) {
        ...
        if (!arr) {
          ...
          if (typeof arr === 'string') data[key] = arr = [arr];
          arr.push(val);
        }
      }
    }
  ...
  StateHandler.prototype.getCrumbs = function (crumbTrail) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var _a = this.parseLink(crumblessUrl),
        state = _a.state,
        data = _a.data,
        hash = _a.hash;
      delete data[state.crumbTrailKey];
      var url = this.getNavigationLink(
        state,
        data,
        hash,
        crumbTrail.slice(0, i)
      );
      crumbs.push(
        new Crumb(data, state, url, crumblessUrl, i + 1 === len, hash)
      );
    }
    return crumbs;
  };
...
var StateNavigator = /** @class */ (function () {
  function StateNavigator(stateInfos, historyManager) {
    ...
    this.states = {};
    this.onBeforeNavigate = function (handler) {
      return _this.onBeforeNavigateCache.onEvent(handler);
    };
    this.offBeforeNavigate = function (handler) {
      return _this.onBeforeNavigateCache.offEvent(handler);
    };
    this.onNavigate = function (handler) {
      return _this.onNavigateCache.onEvent(handler);
    };
    this.offNavigate = function (handler) {
      return _this.onNavigateCache.offEvent(handler);
    };
    if (stateInfos) this.configure(stateInfos, historyManager);
  }
  StateNavigator.prototype.configure = function (stateInfos, historyManager) {
    var _this = this;
    if (this.historyManager) this.historyManager.stop();
    this.historyManager = historyManager
      ? historyManager
      : new HashHistoryManager();
    this.historyManager.init(
      function (url) {
        if (url === void 0) {
          url = _this.historyManager.getCurrentUrl();
        }
        _this.navigateLink(url, undefined, true);
      },
      function (url) {
        return _this.rewriteCache.rewrites[url];
      }
    );
  ...
  StateNavigator.prototype.createStateContext = function (
    state,
    data,
    hash,
    crumbs,
    ...
    stateContext.nextCrumb = new Crumb(
      data,
      state,
      url,
      this.stateHandler.getLink(state, data, hash),
      false,
      hash
    );
  ...
  StateNavigator.prototype.navigate = function (
    stateKey,
    navigationData,
    historyAction
  ) {
  ...
  StateNavigator.prototype.getNavigationLink = function (
    stateKey,
    navigationData,
    hash
  ) {
    ...
    var _a = this.stateContext,
      crumbs = _a.crumbs,
      nextCrumb = _a.nextCrumb;
    var url = this.stateHandler.getLink(
  ...
  StateNavigator.prototype.getNavigationBackLink = function (distance) {
    if (!this.canNavigateBack(distance))
      throw new Error(
        'The distance parameter must be greater than zero and less than or equal to the number of Crumbs (' +
          this.stateContext.crumbs.length +
          ')'
      );
    var _a =
        this.stateContext.crumbs[this.stateContext.crumbs.length - distance],
      url = _a.url,
      state = _a.state,
      data = _a.data;
    this.rewrite(
      ...
      state,
      data,
      this.stateContext.crumbs.slice(
        0,
        this.stateContext.crumbs.length - distance
      )
    );
    return url;
  };
  ...
  StateNavigator.prototype.navigateLink = function (
    url,
    historyAction,
    history,
    suspendNavigation,
    ...
    var _this = this;
    if (historyAction === void 0) {
      historyAction = 'add';
    }
    if (history === void 0) {
      history = false;
    }
    if (suspendNavigation === void 0) {
      suspendNavigation = function (_, resumeNavigation) {
    ...
    if (history && this.stateContext.url === url) return;
    var context = this.stateContext;
    var _a = this.parseLink(url),
      state = _a.state,
      data = _a.data,
      hash = _a.hash,
      crumbs = _a.crumbs;
    for (var id in this.onBeforeNavigateCache.handlers) {
      ...
      if (
        ...
        !handler(state, data, url, history, currentContext)
      )
        return;
    }
    var navigateContinuation = function (asyncData) {
      var nextContext = _this.createStateContext(
        state,
        data,
        hash,
        crumbs,
    ...
    if (currentContext.state)
      currentContext.state.unloading(
        state,
        data,
        url,
        unloadContinuation,
    ...
    else state.navigating(data, url, navigateContinuation, history);
  };
  StateNavigator.prototype.resumeNavigation = function (stateContext) {
    this.stateContext = stateContext;
    var oldState = stateContext.oldState,
      state = stateContext.state,
      data = stateContext.data,
      asyncData = stateContext.asyncData,
      url = stateContext.url,
      crumbs = stateContext.crumbs,
      historyAction = stateContext.historyAction;
    this.rewriteCache.rewrites = {};
    ...
    for (var id in this.onNavigateCache.handlers) {
      if (stateContext === this.stateContext)
        this.onNavigateCache.handlers[id](
          oldState,
          state,
          data,
          asyncData,
  ...
  StateNavigator.prototype.rewrite = function (
    url,
    state,
    navigationData,
    crumbs,
    ...
    if (url && !this.rewriteCache.rewrites[url]) {
      ...
      var rewrittenNavigation =
        (_a = state.rewriteNavigation) === null || _a === void 0
          ? void 0
          : _a.call(state, navigationData);
      if (rewrittenNavigation) {
        ...
        var stateKey = rewrittenNavigation.stateKey,
          navigationData = rewrittenNavigation.navigationData,
          hash = rewrittenNavigation.hash;
        state = this.states[stateKey];
        if (state) {
          var rewrittenUrl = this.stateHandler.getLink(
            state,
            navigationData,
            hash,
            crumbs
  ...
  StateNavigator.prototype.parseLink = function (url) {
    var _a = this.stateHandler.parseLink(url),
      state = _a.state,
      data = _a.data,
      hash = _a.hash;
    var _b = data,
      _c = state.crumbTrailKey,
      crumbs = _b[_c],
      data = __rest(_b, [typeof _c === 'symbol' ? _c : _c + '']);
  ...
  StateNavigator.prototype.start = function (url, serverRendered) {
    var _a;
    if (serverRendered === void 0) {
      serverRendered = false;
    }
    if (
      url == null &&
      serverRendered &&
      !this.historyManager.disabled &&
      ((_a = window.history.state) === null || _a === void 0
        ? void 0
        : _a.navigationLink)
    ) {
