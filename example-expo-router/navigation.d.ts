 ...
 */
export class State<Key extends string = string, Data extends object = any>
  implements StateInfo<Key>
{
  /**
   * Gets the unique key
   */
  ...
  rewriteNavigation(
    ...
    | { stateKey: string; navigationData?: any; hash?: string }
    | null
    | undefined;
}
...
export interface HistoryManager {
   ...
   */
  init(
    navigateHistory: (url?: string) => void,
...
export interface FluentNavigator<
  NavigationInfo extends { [index: string]: any } = any,
...
> {
   ...
   */
  navigate<StateKey extends keyof NavigationInfo>(
    stateKey: StateKey,
    navigationData?: null | NavigationInfo[StateKey],
    hash?: string
  ): FluentNavigator<NavigationInfo, StateKey>;
  /**
   ...
   */
  refresh(
    navigationData?: null | NavigationInfo[Key],
    hash?: string
  ): FluentNavigator<NavigationInfo, Key>;
}
...
> {
  ...
  stateContext: StateContext<
    Key & string,
    Key extends keyof NavigationInfo ? NavigationInfo[Key] : any
  >;
  /**
   ...
   */
  states: {
    [Key in keyof NavigationInfo & string]: State<Key, NavigationInfo[Key]>;
  };
  /**
  ...
  constructor(
    stateInfos?:
      | StateInfo<(keyof NavigationInfo & string) | string>[]
      | StateNavigator<NavigationInfo>,
    historyManager?: HistoryManager
  );
  /**
   ...
   * @throws A mandatory route parameter has not been supplied a value
   */
  navigate<StateKey extends keyof NavigationInfo & string>(
    stateKey: StateKey,
    navigationData?: null | NavigationInfo[StateKey],
    historyAction?: 'add' | 'replace' | 'none'
  ): void;
   ...
   * NavigationData that cannot be converted to a String
   */
  getNavigationLink<StateKey extends keyof NavigationInfo & string>(
    stateKey: StateKey,
    navigationData?: null | NavigationInfo[StateKey],
    hash?: string
  ): string;
  ...
  parseLink<StateKey extends keyof NavigationInfo>(
    url: string
  ): {
    state: State<StateKey & string, NavigationInfo[StateKey]>;
    data: NavigationInfo[StateKey];
    hash: string;
    crumbs: Crumb[];
  };
  /**
   ...
   */
  fluent<B extends boolean>(
    withContext?: B
  ): FluentNavigator<NavigationInfo, B extends true ? Key : string>;
  /**
